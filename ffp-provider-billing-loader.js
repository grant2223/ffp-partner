...
// ─── Stripe Connect ────────────────────────────────────────────────────────
// NOT HERE. Connecting Stripe lives in ONE place only: the Connect Payments panel
// (#panel-payments in index.html, rendered by renderPaymentsCard()). This file used to
// carry a second card + its own startConnect() reading the cached window.FFP_PROVIDER
// .payments_status, which showed a duplicate AND a stale banner. Both deleted — do not
// re-add a connect button to Payments & Invoices.
...
async function renderPayments() {
  ...
  // The ?stripe=… bounce-back toast is handled once in index.html, which then strips the
  // param and opens Connect Payments. This file no longer touches the Stripe flow.
